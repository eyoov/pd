...
//#endif
    ...
    OUR_NUMBER_OF_ROWS = NUMBER_OF_ROWS / (NUMBER_OF_PROCESSES - 1); // change this state on minus
    if(OUR_RANK == NUMBER_OF_PROCESSES - 2)
    {
        OUR_NUMBER_OF_ROWS += NUMBER_OF_ROWS % (NUMBER_OF_PROCESSES - 1);
    }
...
//#ifdef SHOW_RESULTS
        for(current_rank = 0; current_rank <= NUMBER_OF_PROCESSES - 2; current_rank++) //change this state on - 1
